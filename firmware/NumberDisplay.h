...
class NumberDisplay : public Adafruit_ST77xx, public Multiplexer {
private:
  int8_t values[8];
public:
  NumberDisplay(int8_t ChipSelects[], uint8_t Digits, int8_t cs, int8_t dc, int8_t rst);
  NumberDisplay(int8_t ChipSelects[], uint8_t Digits, int8_t cs, int8_t dc, int8_t mosi, int8_t sclk, int8_t rst = -1);
   ...
   * digit: zero-based index, selecting the digits. 0 means right.
   */
  void DrawNumber(int n);
  void DrawNumber(int n, uint8_t digit);
};
